...
- Every converter throws `InvalidArgumentException` for values it cannot convert; only the non-obvious throw conditions are called out per method. Where a converter returns a value instead of throwing, its page carries a **Limitations** section saying so — `toInt` and `toFloat` have one each, at the int64 and float limits.
