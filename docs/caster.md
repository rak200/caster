...
- [`toString` / `tryToString`](#tostring--trytostring)
- [`toInt` / `tryToInt`](#toint--trytoint)
- [`toFloat` / `tryToFloat`](#tofloat--trytofloat)
- [`toBool` / `tryToBool`](#tobool--trytobool)
- [`toArray` / `tryToArray`](#toarray--trytoarray)
- [`toNumber` / `tryToNumber`](#tonumber--trytonumber)
- [`toDateTime` / `tryToDateTime`](#todatetime--trytodatetime)
- [`toEnum` / `tryToEnum`](#toenum--trytoenum)
- [`toCollection` / `tryToCollection`](#tocollection--trytocollection)
- [`cast` / `tryCast`](#cast--trycast)
- [`toJson` / `tryToJson`](#tojson--trytojson)

---

## `toString` / `tryToString`
...
Caster::toDateTime('not a date');            // throws InvalidArgumentException
Caster::toDateTime(null);                    // throws InvalidArgumentException
Caster::tryToDateTime('not a date');         // null
```
...
Caster::tryToEnum(mixed $value, class-string<T> $enumClass = UnitEnum::class): ?T
```

`tryToEnum` returns `null` for **any** failure — including an `$enumClass` that is not an enum.
...
Caster::tryToEnum('Clubs', Suit::class);          // null
```
...
## `toCollection` / `tryToCollection`

```php
Caster::toCollection(mixed $value): iterable
Caster::tryToCollection(mixed $value): ?iterable
```
...
Caster::tryToCollection('abc');          // null
```
...
Caster::tryCast(Castable $value): string|int|float|bool|array|BcMath\Number|DateTimeImmutable|UnitEnum|Traversable|null
```
...
Throws `InvalidArgumentException` when the object implements only the marker `Castable` interface; `tryCast` returns `null` in that case.
...
Caster::tryToJson(mixed $value, int $flags = JSON_PRETTY_PRINT): ?string
```
